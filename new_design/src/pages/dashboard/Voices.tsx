...
export default function Voices() {
  ...
  const load = useCallback(async () => {
    if (!user) return;
    setLoading(true);
    const { data, error } = await action("voices.list", {});
    if (error) toast.error(error);
    setRows((data as Voice[]) ?? []);
    setLoading(false);
  }, [user, action]);
  ...
  const uploadBlob = async (blob: Blob, title: string, duration?: number) => {
    ...
    try {
      ...
      const { error } = await action("voices.add_from_base64", {
        name: title,
        base64,
        mime: blob.type,
  ...
  const saveTranscript = async () => {
    if (!selected) return;
    const { error } = await action("voices.update_transcript", {
      id: selected.id,
  ...
  const summarize = async () => {
    ...
    setBusy(true);
    try {
      await action("voices.update_transcript", { id: selected.id, transcript: transcriptDraft });
      const { data, error } = await action("voices.summarize", { id: selected.id });
      if (error) throw new Error(error);
  ...
  const del = async (v: Voice) => {
    const { error } = await action("voices.delete", { id: v.id });
    if (error) return toast.error(error);
    if (selected?.id === v.id) setSelected(null);
    load();
  };

  const renameVoice = async (id: string, title: string) => {
    const { error } = await action("voices.rename", { id, title });
    if (error) return toast.error(error);
    load();
  };
            ...
            <div className="p-6 space-y-5 max-w-3xl">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <Input
                    ...
                    onBlur={(e) => renameVoice(selected.id, e.target.value)}
              ...
              {selected.url && (
                <div className="rounded-lg border border-border bg-card p-3 flex items-center gap-3">
                  ...
                  <audio ref={audioRef} src={selected.url} onEnded={() => setPlaying(false)} className="flex-1" controls />
                </div>
              )}
