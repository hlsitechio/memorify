...
export default function MemoryDetail() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    // Fetch memory by mem_id or id
    const { data, error } = await action("memory.get", { id: memId });
    if (error || !data) {
      toast.error(error ?? "Memory not found");
      setLoading(false);
      return;
    }
    const r = data as MemoryRow;
    setRow(r);
    setForm({
    ...
    // Fetch versions
    const { data: vs } = await action("memory.versions", { id: r.id });
  ...
  const save = async () => {
    ...
    const { error } = await action("memory.update", {
      id: row.id,
  ...
  const archive = async (value: boolean) => {
    if (!row) return;
    const { error } = await action("memory.archive", { ids: [row.id], archived: value });
    if (error) return toast.error(error);
  ...
  const del = async () => {
    ...
    const { error } = await action("memory.delete", { ids: [row.id] });
    if (error) return toast.error(error);
