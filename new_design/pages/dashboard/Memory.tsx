...
export default function Memory() {
  ...
  const load = useCallback(async () => {
    if (!user) return;
    setLoading(true);
    const { data, error } = await action("memory.list", { include_archived: true });
  ...
  // namespace; org workspace sees everything that isn't an agent namespace.
  ...
  const create = async () => {
    ...
    const { error } = await action("memory.add", {
  ...
  const aiSuggest = async () => {
    ...
    setAiBusy(true);
    try {
      const { data, error } = await action("memory.suggest", { input: aiInput });
  ...
  const archive = async (ids: string[], value: boolean) => {
    const { error } = await action("memory.archive", { ids, archived: value });
    if (error) return toast.error(error);
  ...
  const del = async (ids: string[]) => {
    const { error } = await action("memory.delete", { ids });
    if (error) return toast.error(error);
  ...
  const moveCategory = async (ids: string[], target: string) => {
    const { error } = await action("memory.move", { ids, category: target });
    if (error) return toast.error(error);
            ...
            {loading ? (
              ...
              filtered.map((r) => (
                <div key={r.id} className="grid grid-cols-[40px_140px_110px_120px_1fr_140px_110px_40px] items-center px-4 py-3 border-b border-border last:border-0 hover:bg-secondary/30 transition-colors cursor-pointer" onClick={() => navigate(`/dashboard/memory/${r.mem_id ?? r.id}`)}>
                  ...
                  <div className="font-mono text-[11px] text-primary truncate" title={r.mem_id ?? ""}>{r.mem_id ?? r.id.slice(0, 8)}</div>
