...
const sections: { label: string; items: { to: string; label: string; icon: typeof Home; end?: boolean }[] }[] = [
  ...
  {
    label: "Chat",
    items: [{ to: "/dashboard/copilot", label: "Copilot", icon: Bot }],
  },
  {
    label: "Build",
    items: [
  ...
  },
  {
    ...
    items: [
      ...
      { to: "/dashboard/mind-map", label: "Mind Map", icon: Share2 },
...
function MainNav({ collapsed, onToggleChat }: { collapsed: boolean; onToggleChat: () => void }) {
  return (
      ...
      {sections.map((section) => (
        <div key={section.label} className="space-y-0.5">
          ...
          {section.items.map((it) => {
            // Special handling for Copilot - open chat sidebar instead of navigating
            if (it.to === "/dashboard/copilot") {
              return (
                <button
                  key={it.to}
        ...
        {chatOpen && !collapsed ? (
          ...
          <>
            ...
            <div key={inDocs ? "docs" : "main"} className="flex-1 min-h-0 flex flex-col animate-nav-swap">
              {inDocs ? <DocsNav collapsed={collapsed} /> : <MainNav collapsed={collapsed} onToggleChat={toggleChat} />}
            </div>
            ...
            <div className="p-2 border-t border-border space-y-1">
              ...
              <div className="flex items-center justify-between px-2 py-1 border-t border-border">
                <div className="flex items-center gap-2">
                  <ShieldCheck className="h-4 w-4 text-green-500" title="Clerk Auth: Connected" />
                  <ShieldCheck className="h-4 w-4 text-blue-500" title="Neon Auth: Connected" />
                </div>
                <Wifi className="h-4 w-4 text-green-500" title="Neon Data API: Online" />
              </div>
              <button
                onClick={async () => {
                  await signOut();
