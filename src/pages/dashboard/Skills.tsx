...
export default function Skills() {
  ...
  const upsert = async () => {
    ...
    if (editing) {
      const { error } = await supabase.from("skills").update({
        name: form.name, slug, description: form.description, prompt: form.prompt, model: form.model, status: form.status,
  ...
  const runTry = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("skill-run", {
        body: { skill_id: tryOpen.id, input: tryInput },
      });
      if (error) throw error;
      if (data?.error) throw new Error(data.error);
      setTryOutput(data?.output ?? "");
    } catch (e: any) {
  ...
  const addAsPlugin = async (s: Skill) => {
    ...
    const { error } = await supabase.from("plugins").insert({
      user_id: user.id, name: s.name, kind: "skill", ref_id: s.id, config: { slug: s.slug },
    });
    if (error) return toast.error(error.message);
