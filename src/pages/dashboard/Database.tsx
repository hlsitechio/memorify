...
export default function DatabasePage() {
  ...
  const handleSmartImport = async () => {
    ...
    setImporting(true);
    try {
      const { data, error } = await supabase.functions.invoke("collection-ai", {
        body: { mode: "import", text: importText, hint: importHint },
      });
      if (error) throw error;
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) throw new Error("Not signed in");

      const slugBase = (data.collection_name || "collection").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
      const slug = `${slugBase}-${Date.now().toString(36)}`;
      const { data: col, error: ce } = await supabase.from("collections").insert({
        user_id: user.id,
      ...
      const rows = (data.items ?? []).map((d: any) => ({
        collection_id: col.id, user_id: user.id, data: d,
      }));
      if (rows.length) {
        const { error: ie } = await supabase.from("collection_items").insert(rows);
        if (ie) throw ie;
      }
      toast.success(`Imported ${rows.length} items into "${col.name}"`);
  ...
  const handleAsk = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("collection-ai", {
        body: { mode: "query", question: askText, schema: active.schema, sample: items.slice(0, 3).map((i) => i.data) },
      });
      if (error) throw error;
  ...
  const handleAddBlank = async () => {
    ...
    const { error } = await supabase.from("collection_items").insert({
      collection_id: active.id, user_id: user.id, data: blank,
    });
    if (error) return toast.error(error.message);
    loadItems(active.id);
  };
